let handler = async (m, { args }) => {
  ...
  for (let i = 0; i < spinResult.length; i++) {
    ...
    if (rand <= winProb) {
      ...
      let winAmount = Math.floor(Math.random() * 50000) + 10000
      if (i === 2) {

        winAmount = Math.floor(Math.random() * 1000000000) + 100000000
      }
      poinAkhir += winAmount
      ...
      let loseAmount = Math.floor(Math.random() * 1000000) + 25000
